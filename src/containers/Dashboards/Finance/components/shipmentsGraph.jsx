...
const ShipmentsGraph = ({ theme, dir, name, data }) => {
  ...
  useEffect(() => {
    ...
    modifiedArray?.forEach((e1, i1) =>
      monthlyArray?.forEach((e2, i2) => {
        ...
        if (week === e2.id) {
          monthlyArray[i2].shipments += e1[1];
        }
      })
    );
    ...
    modifiedYearlyArray?.forEach((e1, i1) =>
      lastTenYears?.forEach((e2, i2) => {
        ...
        if (year === e2.year) {
          lastTenYears[i2].shipments += e1[1];
        }
      })
    );
  }, [finalDate]);
